    ...
    <div className="modal-backdrop" onMouseDown={onClose}>
      <div className="modal" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="modal-grid">
          ...
          <label>Tags<select multiple value={form.tags || []} onChange={(event) => setForm({ ...form, tags: Array.from(event.target.selectedOptions).map((option) => option.value) })}>{tags.filter((tag) => tag.active).map((tag) => <option key={tag.id} value={tag.name}>{tag.name}</option>)}</select><small className="field-help">Hold Ctrl/Cmd to select multiple tags.</small></label>
        </div>
