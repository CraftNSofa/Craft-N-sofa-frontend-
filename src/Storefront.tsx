...
export default function Storefront() {
  ...
  const carouselProducts = useMemo(() => {
    ...
    const copies = filtered.length < 4 ? Math.ceil(4 / filtered.length) : 2;
    return Array.from({ length: copies }, () => filtered).flat();
  }, [filtered]);

  useEffect(() => {
    if (isCategoryPage || loading || !carouselProducts.length) return;
  ...
  const scrollCollectionRight = () => { const row = collectionGridRef.current; if (row) row.scrollBy({ left: Math.max(row.clientWidth * 0.78, 280), behavior: 'smooth' }); };
...
<section className="collection" id="collection"><h2 className="collection-simple-title">Double Bed and Sofa Cumbed</h2>{loading ? <div className="store-loading">Loading the collection…</div> : <div className="collection-carousel"><div ref={collectionGridRef} className="store-grid">{carouselProducts.map((product, index) => <ProductCard key={`${String(product.id)}-${index}`} product={product} onAdd={add} onView={setQuickViewProduct} />)}</div>{carouselProducts.length > 4 && <button type="button" className="collection-next" onClick={scrollCollectionRight} aria-label="Show more products"><ArrowRight size={19} /></button>}</div>}</section>
