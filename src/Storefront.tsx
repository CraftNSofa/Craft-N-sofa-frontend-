...
export default function Storefront() {
  ...
  return <div className="storefront" id="top">
    ...
    <main>
      <section className="store-banner-carousel" aria-label="Craft N Sofa promotions">{banners.length ? <>{banners.map((banner, index) => <img key={banner.id} className={`store-carousel-slide ${index === activeBanner ? 'active' : ''}`} src={banner.image_url} alt={banner.alt_text} />)}{banners.length > 1 && <div className="store-carousel-controls" aria-label="Banner navigation">{banners.map((banner, index) => <button key={banner.id} className={index === activeBanner ? 'active' : ''} aria-label={`Show banner ${index + 1}`} onClick={() => setActiveBanner(index)} />)}</div>}</> : <div className="store-banner-empty"><span className="store-kicker">CRAFT N SOFA</span><h1>Make room for <em>good living.</em></h1><a className="store-button dark" href="#collection">Explore collection <ArrowRight size={16} /></a></div>}</section>
