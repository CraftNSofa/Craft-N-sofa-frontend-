...
export default function Storefront() {
  ...
  return <div className="storefront">
    <header className="store-header"><a className="store-logo" href="/"><span>C</span>RAFT N SOFA</a><nav className={menuOpen ? 'store-nav open' : 'store-nav'}><a href="#collection" onClick={() => { setActiveCategory('All pieces'); setMenuOpen(false); }}>Collection</a>{categories.slice(0, 5).map(category => <a href="#collection" key={category.id} onClick={() => { setActiveCategory(category.name); setMenuOpen(false); }}>{category.name}</a>)}<a href="#story" onClick={() => setMenuOpen(false)}>Our story</a><a href="#contact" onClick={() => setMenuOpen(false)}>Contact</a></nav><button className="store-cart" onClick={() => setCartOpen(true)}><ShoppingBag size={18} /><span>{cart.reduce((sum, line) => sum + line.quantity, 0)}</span></button><button className="store-menu" onClick={() => setMenuOpen(!menuOpen)}><Menu size={21} /></button></header>
    <main>
    ...
    <footer className="store-footer" id="contact"><div className="store-logo"><span>C</span>RAFT N SOFA</div><p>Furniture with feeling.</p><small>© {new Date().getFullYear()} Craft N Sofa</small></footer>
