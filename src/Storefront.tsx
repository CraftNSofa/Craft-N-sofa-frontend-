...
<section className="collection" id="collection"><h2 className="collection-simple-title">Double Bed and Sofa Cumbed</h2>{loading ? <div className="store-loading">Loading the collection…</div> : <div ref={collectionGridRef} className="store-grid">{carouselProducts.map((product, index) => <ProductCard key={`${String(product.id)}-${index}`} product={product} onAdd={add} onView={setQuickViewProduct} />)}</div>}</section>
