...
function ProductCard({ product, onAdd }: { product: Product; onAdd: (product: Product) => void }) {
  ...
  return <article className="store-product">
    <button className="product-visual" onClick={() => onAdd(product)} aria-label={`Add ${product.name} to bag`}>
      ...
      <span className="quick-add">Add to bag <Plus size={15} /></span>
    </button>
...
<section className="collection" id="collection"><div className="collection-reference-heading"><span className="store-kicker">{activeCategory === 'All pieces' ? 'THE COLLECTION' : 'COLLECTION'}</span><h2>{activeCategory === 'All pieces' ? 'Craft N Sofa Furniture' : `${activeCategory} Furniture`}</h2><button className="view-all-button" onClick={() => setActiveCategory('All pieces')}>View all</button><span className="piece-count">{filtered.length} pieces</span></div><div className="category-tabs"><button className={activeCategory === 'All pieces' ? 'active' : ''} onClick={() => setActiveCategory('All pieces')}>All pieces</button>{categories.map(category => <button className={activeCategory === category.name ? 'active' : ''} key={category.id} onClick={() => setActiveCategory(category.name)}>{category.name}</button>)}</div>{loading ? <div className="store-loading">Loading the collection…</div> : <div className="store-grid">{filtered.map(product => <ProductCard key={String(product.id)} product={product} onAdd={add} />)}</div>}</section>
