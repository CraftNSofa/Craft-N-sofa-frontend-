...
import { createExpense, deleteCategory, deleteProduct as deleteRemoteProduct, getCurrentUser, loadCategories, loadExpenses, loadOrders, loadProducts, signInAdmin, signOutAdmin, subscribeToOrders, updateOrderStatus as updateRemoteOrderStatus, upsertCategory, saveProduct as saveRemoteProduct, uploadProductImage } from './lib/commerce';
import { getSupabaseClient } from './config/supabase';

type Tab = 'overview' | 'products' | 'orders' | 'finance';
...
type ProductForm = { id?: string; name: string; description: string; price: number; discount_price: number | null; original_price: number | null; category: string; image_url: string; images: string[]; colors: string[]; stock: number; featured: boolean; cost_price: number; image_file?: File };
...
function App() {
  ...
  useEffect(() => {
    ...
    getCurrentUser().then(async user => {
      ...
      setSession(true);
      const [remoteProducts, remoteCategories, remoteOrders, remoteExpenses] = await Promise.all([loadProducts(), loadCategories(), loadOrders(), loadExpenses()]);
      if (!active) return;
      setProducts(remoteProducts);
      setCategories(remoteCategories.map(category => ({ id: category.id, name: category.name, description: category.description || '', active: category.active })));
      setOrders(remoteOrders.map(order => ({ id: order.id, customer: order.customer_name, location: String(order.shipping_address?.city || 'Online'), items: order.order_items?.reduce((sum, item) => sum + item.quantity, 0) || 0, total: Number(order.total), cost: order.order_items?.reduce((sum, item) => sum + Number(item.cost_price) * item.quantity, 0) || 0, status: ({ pending: 'Pending', confirmed: 'Confirmed', processing: 'Processing', shipped: 'Shipped', delivered: 'Delivered', cancelled: 'Cancelled' } as Record<string, OrderStatus>)[order.status] || 'Pending', date: order.created_at.slice(0, 10), payment: order.payment_status === 'paid' ? 'Paid' : 'Pending' })));
      setExpenses(remoteExpenses.map(expense => ({ id: expense.id, title: expense.title, category: expense.category, amount: Number(expense.amount), date: expense.expense_date })));
      setRemoteReady(true);
    }).catch(() => undefined);
    return () => { active = false; };
  }, []);
  ...
  const openNewProduct = () => setProductModal({ name: '', description: '', price: 0, discount_price: null, original_price: null, category: activeCategories[0]?.name || 'Sofas', image_url: '', images: [], colors: [], stock: 0, featured: false, cost_price: 0 });
  const saveProduct = async (data: ProductForm) => {
    try {
      ...
      const item = { ...data, image_url: imageUrl, images: imageUrl ? [imageUrl, ...(data.images || []).filter(image => !image.startsWith('blob:'))] : data.images, id: data.id || undefined, created_at: data.id ? undefined : new Date().toISOString(), updated_at: new Date().toISOString() } as Product;
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${mobileNav ? 'mobile-open' : ''}`}>
      ...
      <div className="sidebar-bottom"><div className="help-card"><div className="help-icon"><Bell size={17} /></div><b>Stay on top of orders</b><p>Turn on notifications when connecting your live store.</p></div><button className="nav-item"><Settings size={18} /><span>Settings</span></button><button className="nav-item logout" onClick={logout}><span className="avatar">AM</span><span className="user-name">Admin Manager</span><ChevronDown size={15} /></button></div>
    </aside>
