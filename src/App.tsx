...
import type { Product, Tag as ProductTag } from './types';
import { INITIAL_SAMPLE_PRODUCTS } from './config/supabase';
import { clearStoreLogo, createExpense, deleteCategory, deleteProduct as deleteRemoteProduct, deleteTag, getCurrentUser, loadCategories, loadExpenses, loadOrders, loadProducts, loadStoreBranding, loadTags, signInAdmin, signOutAdmin, subscribeToOrders, updateOrderStatus as updateRemoteOrderStatus, upsertCategory, upsertTag, saveProduct as saveRemoteProduct, uploadProductImage, uploadStoreLogo } from './lib/commerce';
...
function App() {
  ...
  useEffect(() => {
    ...
    getCurrentUser().then(async user => {
      ...
      setSession(true);
      const results = await Promise.allSettled([loadProducts(), loadCategories(), loadTags(), loadOrders(), loadExpenses(), loadStoreBranding()]);
      if (!active) return;
      const [productResult, categoryResult, tagResult, orderResult, expenseResult, brandingResult] = results;
