...
import type { Banner, Product, Tag as ProductTag } from './types';
import { INITIAL_SAMPLE_PRODUCTS } from './config/supabase';
import { clearStoreLogo, createExpense, deleteCategory, deleteProduct as deleteRemoteProduct, deleteStoreBanner, deleteTag, getCurrentUser, loadBanners, loadCategories, loadExpenses, loadOrders, loadProducts, loadStoreBranding, loadTags, saveStoreContent, uploadSecondaryStoreImage, clearSecondaryStoreImage, signInAdmin, signOutAdmin, subscribeToOrders, updateOrderStatus as updateRemoteOrderStatus, updateStoreBanner, upsertCategory, upsertTag, saveProduct as saveRemoteProduct, uploadProductImage, uploadCategoryImage, uploadStoreBanner, uploadStoreLogo } from './lib/commerce';
import { getSupabaseClient } from './config/supabase';

type Tab = 'overview' | 'products' | 'add-product' | 'add-category' | 'categories' | 'add-tag' | 'tags' | 'orders' | 'finance' | 'settings';
...
function App() {
  ...
  const [storeContent, setStoreContent] = useState({ custom_html: '', custom_css: '', secondary_image_url: null as string | null });
  ...
  useEffect(() => {
    ...
    getCurrentUser().then(async user => {
      ...
      if (brandingResult.status === 'fulfilled') { setBrandLogoUrl(brandingResult.value.logo_url); setStoreContent({ custom_html: brandingResult.value.custom_html || '', custom_css: brandingResult.value.custom_css || '', secondary_image_url: brandingResult.value.secondary_image_url || null }); }
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="page-body">
        ...
        {tab === 'settings' && <StoreSettingsView logoUrl={brandLogoUrl} banners={banners} content={storeContent} onSaveContent={async (html, css) => { try { if (!remoteReady) throw new Error('Connect the admin workspace before saving storefront content'); const saved = await saveStoreContent({ custom_html: html, custom_css: css }); setStoreContent({ custom_html: saved.custom_html, custom_css: saved.custom_css, secondary_image_url: saved.secondary_image_url }); showToast('Storefront content saved'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not save storefront content'); } }} onUploadSecondary={async file => { try { if (!remoteReady) throw new Error('Connect the admin workspace before uploading storefront images'); const saved = await uploadSecondaryStoreImage(file, storeContent.secondary_image_url); setStoreContent({ custom_html: saved.custom_html, custom_css: saved.custom_css, secondary_image_url: saved.secondary_image_url }); showToast('Secondary image uploaded'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not upload secondary image'); } }} onClearSecondary={async () => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing storefront images'); const saved = await clearSecondaryStoreImage(storeContent.secondary_image_url); setStoreContent({ custom_html: saved.custom_html, custom_css: saved.custom_css, secondary_image_url: saved.secondary_image_url }); showToast('Secondary image removed'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not remove secondary image'); } }} onChangeLogo={() => setBrandingModal(true)} onAddBanner={() => setBannerModal(true)} onToggleBanner={async (id, active) => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing banners'); const saved = await updateStoreBanner(id, { active }); setBanners(current => current.map(banner => banner.id === id ? saved : banner)); showToast(active ? 'Banner activated' : 'Banner hidden'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not update banner'); } }} onDeleteBanner={async banner => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing banners'); await deleteStoreBanner(banner.id, banner.image_url); setBanners(current => current.filter(item => item.id !== banner.id)); showToast('Banner removed'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not remove banner'); } }} onRemove={async () => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing branding'); await clearStoreLogo(brandLogoUrl); setBrandLogoUrl(null); showToast('Store logo removed'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not remove logo'); } }} />}
      </div>
    </main>
...
function StoreSettingsView({ logoUrl, banners, content, onChangeLogo, onAddBanner, onToggleBanner, onDeleteBanner, onRemove, onSaveContent, onUploadSecondary, onClearSecondary }: { logoUrl: string | null; banners: Banner[]; content: { custom_html: string; custom_css: string; secondary_image_url: string | null }; onChangeLogo: () => void; onAddBanner: () => void; onToggleBanner: (id: string, active: boolean) => Promise<void>; onDeleteBanner: (banner: Banner) => Promise<void>; onRemove: () => Promise<void>; onSaveContent: (html: string, css: string) => Promise<void>; onUploadSecondary: (file: File) => Promise<void>; onClearSecondary: () => Promise<void> }) {
  return <><section className="welcome-row"><div><h2>Store Settings</h2><p>Manage the identity customers see on your Craft N Sofa website.</p></div></section><section className="settings-grid"><div className="panel settings-card"><div className="panel-heading"><div><span className="eyebrow">BRAND IDENTITY</span><h3>Website logo</h3><p>This logo appears in the public website header and footer.</p></div><Settings size={18} className="muted" /></div><div className="settings-logo-preview">{logoUrl ? <img src={logoUrl} alt="Current store logo" /> : <div className="branding-placeholder">C<span>•</span>S</div>}</div><div className="settings-card-actions"><button className="primary-button" onClick={onChangeLogo}><Upload size={16} /> {logoUrl ? 'Replace logo' : 'Add website logo'}</button>{logoUrl && <button className="danger-button" onClick={onRemove}>Remove logo</button>}</div><small className="settings-help">Recommended: a transparent PNG or SVG. Maximum size 5 MB.</small></div><div className="panel settings-card settings-info"><span className="eyebrow">LIVE PREVIEW</span><h3>Where it appears</h3><div className="settings-location"><span>01</span><div><b>Public header</b><small>Top navigation of your storefront</small></div></div><div className="settings-location"><span>02</span><div><b>Public footer</b><small>Brand signature at the bottom of the website</small></div></div><a href="/" target="_blank" rel="noreferrer" className="secondary-button">Open public store <ArrowUpRight size={15} /></a></div></section><BannerManager banners={banners} onAdd={onAddBanner} onToggle={onToggleBanner} onDelete={onDeleteBanner} /><StoreContentManager content={content} onSaveContent={onSaveContent} onUploadSecondary={onUploadSecondary} onClearSecondary={onClearSecondary} /></>;
}
function BannerManager({ banners, onAdd, onToggle, onDelete }: { banners: Banner[]; onAdd: () => void; onToggle: (id: string, active: boolean) => Promise<void>; onDelete: (banner: Banner) => Promise<void> }) {
  return <section className="panel banner-manager"><div className="panel-heading"><div><span className="eyebrow">STOREFRONT HERO</span><h3>Rotating banners</h3><p>Upload multiple 1500 × 500 style banners. Active banners rotate automatically on the public store.</p></div><button className="primary-button" onClick={onAdd}><ImagePlus size={16} /> Add banner</button></div><div className="banner-admin-grid">{banners.map(banner => <article className="banner-admin-card" key={banner.id}><img src={banner.image_url} alt={banner.alt_text} /><div className="banner-admin-meta"><div><b>{banner.alt_text}</b><small>{banner.active ? 'Visible on storefront' : 'Hidden from storefront'}</small></div><div className="banner-admin-actions"><button className="secondary-button" onClick={() => onToggle(banner.id, !banner.active)}>{banner.active ? 'Hide' : 'Activate'}</button><button className="icon-button danger" onClick={() => onDelete(banner)}><Trash2 size={15} /></button></div></div></article>)}{!banners.length && <div className="empty-state">No banners yet. Add your first storefront banner.</div>}</div><small className="settings-help">Recommended format: 1500 × 500 px. Use JPG, PNG or WebP up to 10 MB.</small></section>;
}
function StoreContentManager({ content, onSaveContent, onUploadSecondary, onClearSecondary }: { content: { custom_html: string; custom_css: string; secondary_image_url: string | null }; onSaveContent: (html: string, css: string) => Promise<void>; onUploadSecondary: (file: File) => Promise<void>; onClearSecondary: () => Promise<void> }) {
  const [html, setHtml] = useState(content.custom_html);
  const [css, setCss] = useState(content.custom_css);
  const [busy, setBusy] = useState(false);
  useEffect(() => { setHtml(content.custom_html); setCss(content.custom_css); }, [content.custom_html, content.custom_css]);
  const preview = `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><style>body{margin:0;font-family:Arial,sans-serif}${css}</style></head><body>${html}</body></html>`;
  const save = async () => { setBusy(true); try { await onSaveContent(html, css); } finally { setBusy(false); } };
  const upload = async (file?: File) => { if (!file || !file.type.startsWith('image/') || file.size > 10 * 1024 * 1024) return; setBusy(true); try { await onUploadSecondary(file); } finally { setBusy(false); } };
  return <section className="panel store-content-manager"><div className="panel-heading"><div><span className="eyebrow">STOREFRONT CONTENT</span><h3>HTML / CSS block</h3><p>Create a custom section below the banner. The preview updates as you type.</p></div><button className="primary-button" onClick={save} disabled={busy}>{busy ? 'Saving…' : 'Save content'}</button></div><div className="content-editor-grid"><div className="content-editors"><label>HTML<textarea value={html} onChange={event => setHtml(event.target.value)} placeholder="<section class='promo'>Fresh arrivals this week</section>" /></label><label>CSS<textarea value={css} onChange={event => setCss(event.target.value)} placeholder=".promo { padding: 48px; text-align: center; background: #f3f1ed; }" /></label></div><div className="content-live-preview"><span className="eyebrow">LIVE PREVIEW</span><iframe title="Custom storefront preview" srcDoc={preview} sandbox="" /></div></div><div className="secondary-image-manager"><div><span className="eyebrow">BELOW THE BANNER</span><h3>Secondary image</h3><p>Optional image shown directly below the rotating banner.</p></div>{content.secondary_image_url ? <div className="secondary-image-row"><img src={content.secondary_image_url} alt="Secondary storefront preview" /><button className="danger-button" onClick={onClearSecondary} disabled={busy}>Remove image</button></div> : <label className="secondary-image-drop"><ImagePlus size={22} /><b>Upload image below the banner</b><small>JPG, PNG or WebP up to 10 MB</small><input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => upload(event.target.files?.[0])} /></label>}</div></section>;
}
