...
function App() {
  ...
  useEffect(() => {
    ...
    getCurrentUser().then(async user => {
      ...
      setSession(true);
      const results = await Promise.allSettled([loadProducts(), loadCategories(), loadOrders(), loadExpenses()]);
      if (!active) return;
      const [productResult, categoryResult, orderResult, expenseResult] = results;
