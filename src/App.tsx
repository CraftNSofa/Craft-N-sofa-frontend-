...
import type { StoreContentBlock, StoreContentBlockKind } from './lib/commerce';
import { INITIAL_SAMPLE_PRODUCTS } from './config/supabase';
import { clearStoreLogo, createExpense, deleteCategory, deleteProduct as deleteRemoteProduct, deleteStoreBanner, deleteTag, getCurrentUser, loadBanners, loadCategories, loadExpenses, loadOrders, loadProducts, loadStoreBranding, loadTags, saveStoreContent, uploadSecondaryStoreImage, clearSecondaryStoreImage, uploadStoreProductBanner, clearStoreProductBanner, saveStoreProductBannerTitle, signInAdmin, signOutAdmin, subscribeToOrders, updateOrderStatus as updateRemoteOrderStatus, upsertCategory, upsertTag, saveProduct as saveRemoteProduct, uploadProductImage, uploadCategoryImage, uploadStoreBanner, replaceStoreBannerAssets, uploadStoreLogo, savePromoCards as saveRemotePromoCards, uploadPromoCardImage, deletePromoCardImage, uploadStoreContentImage, deleteStoreContentImage } from './lib/commerce';
...
type StoreContentData = { custom_html: string; custom_css: string; secondary_image_url: string | null; secondary_image_title: string; product_banner_url: string | null; product_banner_title: string; content_blocks: StoreContentBlock[] };
...
function App() {
  ...
  const [storeContent, setStoreContent] = useState<StoreContentData>({ custom_html: '', custom_css: '', secondary_image_url: null, secondary_image_title: 'Craft N Sofa collection', product_banner_url: null, product_banner_title: 'L-Shaped Corner and Sofa Set', content_blocks: [] });
  ...
  useEffect(() => {
    ...
    getCurrentUser().then(async user => {
      ...
      if (brandingResult.status === 'fulfilled') { setBrandLogoUrl(brandingResult.value.logo_url); setStoreContent({ custom_html: brandingResult.value.custom_html || '', custom_css: brandingResult.value.custom_css || '', secondary_image_url: brandingResult.value.secondary_image_url || null, product_banner_url: brandingResult.value.product_banner_url || null, product_banner_title: brandingResult.value.product_banner_title || 'L-Shaped Corner and Sofa Set', secondary_image_title: brandingResult.value.secondary_image_title || 'Craft N Sofa collection', content_blocks: brandingResult.value.content_blocks || [] }); setPromoCards((brandingResult.value.promo_cards || []).sort((a, b) => a.sort_order - b.sort_order)); }
  ...
  const replaceCategoryImage = async (id: string, file: File) => { try { const category = categories.find(item => item.id === id); if (!category) return; const imageUrl = remoteReady ? await uploadCategoryImage(file) : URL.createObjectURL(file); if (remoteReady) await upsertCategory({ id: category.id, name: category.name, description: category.description, active: category.active, image_url: imageUrl, parent_id: category.parent_id || null }); setCategories(prev => prev.map(item => item.id === id ? { ...item, image_url: imageUrl } : item)); showToast('Category image replaced'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not replace category image'); } };
  ...
  const handleSaveStoreContent = async (next: StoreContentData) => {
    ...
    setStoreContent({ custom_html: saved.custom_html, custom_css: saved.custom_css, secondary_image_url: saved.secondary_image_url, product_banner_url: saved.product_banner_url, secondary_image_title: saved.secondary_image_title, content_blocks: saved.content_blocks });
  ...
  const handleUploadProductBanner = async (file: File) => {
    if (!remoteReady) { setStoreContent(current => ({ ...current, product_banner_url: URL.createObjectURL(file) })); return; }
    const saved = await uploadStoreProductBanner(file, storeContent.product_banner_url);
    setStoreContent(current => ({ ...current, product_banner_url: saved.product_banner_url }));
    showToast('Product banner saved');
  };
  const handleSaveProductBannerTitle = async (title: string) => {
    if (!remoteReady) { setStoreContent(current => ({ ...current, product_banner_title: title })); return; }
    const saved = await saveStoreProductBannerTitle(title);
    setStoreContent(current => ({ ...current, product_banner_title: saved.product_banner_title }));
    showToast('Banner text saved');
  };
  const handleClearProductBanner = async () => {
    if (!remoteReady) { setStoreContent(current => ({ ...current, product_banner_url: null })); return; }
    const saved = await clearStoreProductBanner(storeContent.product_banner_url);
    setStoreContent(current => ({ ...current, product_banner_url: saved.product_banner_url }));
    showToast('Product banner removed');
  };
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="page-body">
        ...
        {tab === 'categories' && <CategoryListView categories={categories} onAdd={() => setCategoryModal(true)} onDelete={id => saveCategories(categories.filter(category => category.id !== id))} onReplaceImage={replaceCategoryImage} />}
        ...
        {tab === 'settings' && <StoreSettingsView logoUrl={brandLogoUrl} banners={banners} tags={tags} content={storeContent} onSaveContent={handleSaveStoreContent} onUploadSecondary={async (file, title) => { try { if (!remoteReady) throw new Error('Connect the admin workspace before uploading storefront images'); await handleUploadSecondary(file, title); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not upload secondary image'); } }} onClearSecondary={async () => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing storefront images'); await handleClearSecondary(); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not remove secondary image'); }}} onUploadProductBanner={async file => { try { if (!remoteReady) throw new Error('Connect the admin workspace before uploading storefront images'); await handleUploadProductBanner(file); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not upload product banner'); } }} onClearProductBanner={async () => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing storefront images'); await handleClearProductBanner(); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not remove product banner'); } }} onSaveProductBannerTitle={async title => { try { if (!remoteReady) throw new Error('Connect the admin workspace before saving storefront text'); await handleSaveProductBannerTitle(title); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not save banner text'); } }} onUploadBlockImage={async (file, previousUrl) => { try { if (!remoteReady) return URL.createObjectURL(file); return await uploadStoreContentImage(file, previousUrl); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not upload block image'); throw error; } }} onChangeLogo={() => setBrandingModal(true)} onAddBanner={() => setBannerModal(true)} onEditBanner={banner => setEditingBanner(banner)} onDeleteBanner={async banner => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing banners'); await deleteStoreBanner(banner.id, banner.image_url, banner.mobile_image_url); setBanners(current => current.filter(item => item.id !== banner.id)); showToast('Banner removed'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not remove banner'); } }} onRemove={async () => { try { if (!remoteReady) throw new Error('Connect the admin workspace before changing branding'); await clearStoreLogo(brandLogoUrl); setBrandLogoUrl(null); showToast('Store logo removed'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not remove logo'); } }} />}
      </div>
    </main>
    ...
    {bannerModal && <BannerUploadModal onClose={() => setBannerModal(false)} onSave={async (desktopFile, mobileFile, altText) => { try { if (!remoteReady) throw new Error('Connect the admin workspace before adding banners'); const saved = await uploadStoreBanner(desktopFile, mobileFile, altText); setBanners(current => [...current, saved]); setBannerModal(false); showToast('Banner added to rotation'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not add banner'); } }} />}
    {editingBanner && <BannerEditModal banner={editingBanner} onClose={() => setEditingBanner(null)} onSave={async (banner, desktopFile, mobileFile, removeMobile, altText) => { try { if (!remoteReady) throw new Error('Connect the admin workspace before editing banners'); const saved = await replaceStoreBannerAssets(banner, desktopFile, mobileFile, removeMobile, altText); setBanners(current => current.map(item => item.id === saved.id ? saved : item)); setEditingBanner(null); showToast('Banner updated'); } catch (error) { showToast(error instanceof Error ? error.message : 'Could not update banner'); } }} />}
...
function StoreSettingsView({ logoUrl, banners, tags, content, onChangeLogo, onAddBanner, onEditBanner, onDeleteBanner, onRemove, onSaveContent, onUploadSecondary, onClearSecondary, onUploadProductBanner, onClearProductBanner, onSaveProductBannerTitle, onUploadBlockImage }: { logoUrl: string | null; banners: Banner[]; tags: AdminTag[]; content: StoreContentData; onChangeLogo: () => void; onAddBanner: () => void; onEditBanner: (banner: Banner) => void; onDeleteBanner: (banner: Banner) => Promise<void>; onRemove: () => Promise<void>; onSaveContent: (content: StoreContentData) => Promise<void>; onUploadSecondary: (file: File, title: string) => Promise<void>; onClearSecondary: () => Promise<void>; onUploadProductBanner: (file: File) => Promise<void>; onClearProductBanner: () => Promise<void>; onSaveProductBannerTitle: (title: string) => Promise<void>; onUploadBlockImage: (file: File, previousUrl?: string | null) => Promise<string> }) {
  return <><section className="welcome-row"><div><h2>Store Settings</h2><p>Manage the identity customers see on your Craft N Sofa website.</p></div></section><section className="settings-grid"><div className="panel settings-card"><div className="panel-heading"><div><span className="eyebrow">BRAND IDENTITY</span><h3>Website logo</h3><p>This logo appears in the public website header and footer.</p></div><Settings size={18} className="muted" /></div><div className="settings-logo-preview">{logoUrl ? <img src={logoUrl} alt="Current store logo" /> : <div className="branding-placeholder">C<span>•</span>S</div>}</div><div className="settings-card-actions"><button className="primary-button" onClick={onChangeLogo}><Upload size={16} /> {logoUrl ? 'Replace logo' : 'Add website logo'}</button>{logoUrl && <button className="danger-button" onClick={onRemove}>Remove logo</button>}</div><small className="settings-help">Recommended: a transparent PNG or SVG. Maximum size 5 MB.</small></div><div className="panel settings-card settings-info"><span className="eyebrow">LIVE PREVIEW</span><h3>Where it appears</h3><div className="settings-location"><span>01</span><div><b>Public header</b><small>Top navigation of your storefront</small></div></div><div className="settings-location"><span>02</span><div><b>Public footer</b><small>Brand signature at the bottom of the website</small></div></div><a href="/" target="_blank" rel="noreferrer" className="secondary-button">Open public store <ArrowUpRight size={15} /></a></div></section><BannerManager banners={banners} onAdd={onAddBanner} onEdit={onEditBanner} onDelete={onDeleteBanner} /><StoreContentManager tags={tags} content={content} onSaveContent={onSaveContent} onUploadSecondary={onUploadSecondary} onClearSecondary={onClearSecondary} onUploadProductBanner={onUploadProductBanner} onClearProductBanner={onClearProductBanner} onSaveProductBannerTitle={onSaveProductBannerTitle} onUploadBlockImage={onUploadBlockImage} /></>;
}
function BannerManager({ banners, onAdd, onEdit, onDelete }: { banners: Banner[]; onAdd: () => void; onEdit: (banner: Banner) => void; onDelete: (banner: Banner) => Promise<void> }) {
  return <section className="panel banner-manager"><div className="panel-heading"><div><span className="eyebrow">MAIN HOMEPAGE BANNER</span><h3>Desktop + mobile banners</h3><p>Upload separate 1500 × 500 desktop and 500 × 500 mobile images for the top banner on the home page.</p></div><button className="primary-button" onClick={onAdd}><ImagePlus size={16} /> Add main banner</button></div><div className="banner-admin-grid">{banners.map(banner => <article className="banner-admin-card" key={banner.id}><div className="banner-admin-previews"><div><small>Desktop</small><img src={banner.image_url} alt={banner.alt_text} /></div><div><small>Mobile</small><img src={banner.mobile_image_url || banner.image_url} alt={`${banner.alt_text} mobile`} /></div></div><div className="banner-admin-meta"><b>{banner.alt_text}</b><div className="banner-admin-actions"><button className="secondary-button" onClick={() => onEdit(banner)}><Edit3 size={14} /> Edit image</button><button className="icon-button danger" onClick={() => onDelete(banner)} title="Remove banner"><Trash2 size={15} /></button></div></div></article>)}{!banners.length && <div className="empty-state">No main banners yet. Add your first desktop/mobile banner.</div>}</div><small className="settings-help">Desktop recommended: 1500 × 500 px. Mobile recommended: 500 × 500 px. JPG, PNG or WebP up to 10 MB.</small></section>;
}
function StoreContentManager({ tags, content, onSaveContent, onUploadSecondary, onClearSecondary, onUploadProductBanner, onClearProductBanner, onSaveProductBannerTitle, onUploadBlockImage }: { tags: AdminTag[]; content: StoreContentData; onSaveContent: (content: StoreContentData) => Promise<void>; onUploadSecondary: (file: File, title: string) => Promise<void>; onClearSecondary: () => Promise<void>; onUploadProductBanner: (file: File) => Promise<void>; onClearProductBanner: () => Promise<void>; onSaveProductBannerTitle: (title: string) => Promise<void>; onUploadBlockImage: (file: File, previousUrl?: string | null) => Promise<string> }) {
  ...
  const [productBannerTitle, setProductBannerTitle] = useState(content.product_banner_title || 'L-Shaped Corner and Sofa Set');
  const [secondaryTitle, setSecondaryTitle] = useState(content.secondary_image_title || 'Craft N Sofa collection');
  const [busy, setBusy] = useState(false);
  const selected = blocks.find(block => block.id === selectedId) || null;
  useEffect(() => { setBlocks(content.content_blocks || []); setSelectedId(content.content_blocks?.[0]?.id || null); setSecondaryTitle(content.secondary_image_title || 'Craft N Sofa collection'); }, [content.content_blocks, content.secondary_image_title]);
  ...
  const addBlock = (kind: StoreContentBlockKind = 'html') => {
    const defaults: Record<StoreContentBlockKind, Partial<StoreContentBlock>> = {
      'main-banner': { title: 'Homepage main banner', html: '', css: '' },
      banner: { title: 'Banner slider', html: '', css: '' },
      html: { title: 'Custom HTML block', html: `<section class="homepage-promo"><h2>Fresh arrivals this week</h2><p>Write your storefront content here.</p></section>`, css: '.homepage-promo { padding: 48px; text-align: center; background: #f3f1ed; }' },
      'tag-carousel': { title: 'Tag product carousel', html: '', css: '', tag_name: '', tag_names: [] },
      'product-carousel': { title: 'Product carousel', html: '', css: '' },
      'product-grid': { title: 'Product grid', html: '', css: '' },
    };
    const block = { id: `block-${uid()}`, kind, image_url: null, image_title: '', active: true, sort_order: blocks.length, ...(defaults[kind] || {}) } as StoreContentBlock;
    setBlocks(current => [...current, block]); setSelectedId(block.id);
  };
  const removeBlock = () => { if (!selectedId) return; const next = blocks.filter(block => block.id !== selectedId); setBlocks(next); setSelectedId(next[0]?.id || null); };
  const moveBlock = (direction: -1 | 1) => { if (!selectedId) return; const index = blocks.findIndex(block => block.id === selectedId); const target = index + direction; if (index < 0 || target < 0 || target >= blocks.length) return; const next = [...blocks]; [next[index], next[target]] = [next[target], next[index]]; setBlocks(next); };
  const save = async () => { setBusy(true); try { await onSaveContent({ ...content, secondary_image_title: secondaryTitle, content_blocks: blocks.map((block, index) => ({ ...block, kind: block.kind || 'html', sort_order: index })) }); } finally { setBusy(false); } };
  const uploadBlockImage = async (file?: File) => { if (!file || !selected) return; const imageUrl = await onUploadBlockImage(file, selected.image_url); updateBlock({ image_url: imageUrl }); };
  const uploadSecondary = async (file?: File) => { if (file) await onUploadSecondary(file, secondaryTitle); };
  const selectedKind = selected?.kind || 'html';
  const blockPreview = selected ? `<!doctype html><html><head><style>body{margin:0;font-family:Arial,sans-serif;color:#222}*{box-sizing:border-box}${selected.css}</style></head><body>${selected.html || `<div style="padding:48px;text-align:center;background:#f3f1ed"><strong>${selected.title}</strong><p>This reusable block will render on the storefront.</p></div>`}</body></html>` : '';
  const labelFor = (block: StoreContentBlock) => block.kind === 'main-banner' ? 'Homepage main banner' : block.kind === 'banner' ? 'Banner slider' : block.kind === 'tag-carousel' ? `Tag carousel${block.tag_name ? ` · ${block.tag_name}` : ''}` : block.kind === 'product-carousel' ? 'Product carousel' : block.kind === 'product-grid' ? 'Product grid' : 'HTML / CSS';
  return <><section className="panel content-block-manager"><div className="panel-heading"><div><span className="eyebrow">HOMEPAGE SECTION BUILDER</span><h3>Build the page in any order</h3><p>Add banners and product carousels between HTML blocks, then move them up or down to control the storefront layout.</p></div><div className="button-row"><button className="secondary-button" onClick={() => addBlock('main-banner')}><ImagePlus size={15} /> Main banner</button><button className="secondary-button" onClick={() => addBlock('banner')}><ImagePlus size={15} /> Slider</button><button className="secondary-button" onClick={() => addBlock('tag-carousel')}><Tag size={15} /> Tag products</button><button className="secondary-button" onClick={() => addBlock('product-carousel')}><ShoppingBag size={15} /> Products</button><button className="secondary-button" onClick={() => addBlock('product-grid')}><ShoppingBag size={15} /> Product grid</button><button className="primary-button" onClick={() => addBlock('html')}><Plus size={15} /> HTML block</button></div></div><div className="content-block-layout"><aside className="content-block-list">{blocks.map((block, index) => <button className={`content-block-item ${selectedId === block.id ? 'selected' : ''}`} key={block.id} onClick={() => setSelectedId(block.id)}><span className="content-block-number">{String(index + 1).padStart(2, '0')}</span><span><b>{block.title || 'Untitled block'}</b><small>{labelFor(block)}</small></span><Edit3 size={14} /></button>)}{!blocks.length && <div className="content-block-empty"><Code2 size={18} /><span>No homepage blocks saved yet.</span><button className="secondary-button" onClick={() => addBlock('banner')}>Add first banner</button></div>}</aside><div className="content-block-editor">{selected ? <><div className="content-block-toolbar"><label>Block title<input value={selected.title} onChange={event => updateBlock({ title: event.target.value })} placeholder="Homepage section" /></label><label className="content-block-active"><input type="checkbox" checked={selected.active} onChange={event => updateBlock({ active: event.target.checked })} /> Active on storefront</label><button className="icon-button" onClick={() => moveBlock(-1)} disabled={blocks.findIndex(block => block.id === selectedId) === 0} title="Move block up"><ChevronDown size={16} style={{ transform: 'rotate(180deg)' }} /></button><button className="icon-button" onClick={() => moveBlock(1)} disabled={blocks.findIndex(block => block.id === selectedId) === blocks.length - 1} title="Move block down"><ChevronDown size={16} /></button><button className="danger-button" onClick={removeBlock}><Trash2 size={14} /> Delete block</button></div>{(selectedKind === 'main-banner' || selectedKind === 'banner' || selectedKind === 'tag-carousel' || selectedKind === 'product-carousel' || selectedKind === 'product-grid') && <div className="block-type-settings"><span className="eyebrow">BLOCK TYPE</span><h3>{labelFor(selected)}</h3>{(selectedKind === 'main-banner' || selectedKind === 'banner') && <p>Upload images directly in this block. Desktop uses 1500 × 500 and mobile uses 500 × 500.</p>}{selectedKind === 'product-carousel' && <p>Select one or more tags below. Products carrying any checked tag will run in this automatic carousel. If none are checked, all published products will show.</p>}{selectedKind === 'product-grid' && <p>This block renders every published product in a responsive grid: four cards on desktop and two cards per row on mobile.</p>}{(selectedKind === 'tag-carousel' || selectedKind === 'product-carousel') && <div className="tag-checkbox-picker"><b>Choose product tags</b><div>{tags.filter(tag => tag.active).map(tag => { const checked = (selected.tag_names || (selected.tag_name ? [selected.tag_name] : [])).includes(tag.name); return <label key={tag.id}><input type="checkbox" checked={checked} onChange={event => { const current = selected.tag_names || (selected.tag_name ? [selected.tag_name] : []); const next = event.target.checked ? [...current, tag.name] : current.filter(name => name !== tag.name); updateBlock({ tag_names: next, tag_name: next[0] || '' }); }} /> <span>{tag.name}</span></label>; })}</div><small>Products carrying any checked tag will appear. You may select multiple tags.</small></div>}</div>}{(selectedKind === 'main-banner' || selectedKind === 'banner') && <div className="block-banner-editor"><label>Desktop image · 1500 × 500<div className="secondary-image-drop">{selected.image_url ? <img src={selected.image_url} alt="Desktop block banner" /> : <><ImagePlus size={22} /><b>Upload desktop banner</b></>}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={async event => { const file = event.target.files?.[0]; if (file) updateBlock({ image_url: await onUploadBlockImage(file, selected.image_url) }); }} /></div></label><label>Mobile image · 500 × 500<div className="secondary-image-drop block-mobile-image">{selected.mobile_image_url ? <img src={selected.mobile_image_url} alt="Mobile block banner" /> : <><ImagePlus size={22} /><b>Upload mobile banner</b></>}<input type="file" accept="image/png,image/jpeg,image/webp" onChange={async event => { const file = event.target.files?.[0]; if (file) updateBlock({ mobile_image_url: await onUploadBlockImage(file, selected.mobile_image_url) }); }} /></div></label><small className="settings-help">These images belong only to this block. Save the homepage order after uploading.</small></div>}{selectedKind === 'html' && <div className="content-editor-grid"><div className="content-editors"><label>HTML<textarea value={selected.html} onChange={event => updateBlock({ html: event.target.value })} placeholder="<section class='promo'>Fresh arrivals this week</section>" /></label><label>CSS<textarea value={selected.css} onChange={event => updateBlock({ css: event.target.value })} placeholder=".promo { padding: 48px; text-align: center; background: #f3f1ed; }" /></label><div className="content-inline-image"><div><label>Block image title<input value={selected.image_title} onChange={event => updateBlock({ image_title: event.target.value })} placeholder="Made for everyday living" /></label><small>Optional image shown after the HTML.</small></div><label className="secondary-image-drop compact"><Upload size={18} /><b>{selected.image_url ? 'Replace block image' : 'Upload block image'}</b><input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => uploadBlockImage(event.target.files?.[0])} /></label></div></div><div className="content-live-preview"><span className="eyebrow">LIVE PREVIEW</span><iframe title={`Preview ${selected.title}`} srcDoc={blockPreview} sandbox="" /></div></div>}<div className="content-editor-actions"><button className="primary-button" onClick={save} disabled={busy}>{busy ? 'Saving…' : 'Save homepage order'}</button></div></> : <div className="content-no-selection"><Code2 size={26} /><h3>Start your homepage layout</h3><p>Add a banner, tag carousel, product carousel, or HTML/CSS block.</p><button className="primary-button" onClick={() => addBlock('banner')}><Plus size={15} /> Add first block</button></div>}</div></div></section><div className="secondary-image-manager product-banner-manager"><div><span className="eyebrow">PRODUCT BANNER</span><h3>Banner below product cards</h3><p>Upload the 1500 × 500 banner shown directly below the first homepage product carousel.</p></div><label>Text below banner<input className="secondary-image-title" value={productBannerTitle} onChange={event => setProductBannerTitle(event.target.value)} placeholder="L-Shaped Corner and Sofa Set" /><button className="secondary-button" onClick={() => void onSaveProductBannerTitle(productBannerTitle)}>Save banner text</button></label>{content.product_banner_url ? <div className="secondary-image-row"><div><img src={content.product_banner_url} alt="Product banner preview" /><small>1500 × 500 recommended</small></div><label className="secondary-image-drop compact"><Upload size={18} /><b>Replace banner</b><input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => { const file = event.target.files?.[0]; if (file) void onUploadProductBanner(file); }} /></label><button className="danger-button" onClick={() => void onClearProductBanner()}>Remove banner</button></div> : <label className="secondary-image-drop"><ImagePlus size={22} /><b>Upload 1500 × 500 banner</b><small>JPG, PNG or WebP up to 10 MB</small><input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => { const file = event.target.files?.[0]; if (file) void onUploadProductBanner(file); }} /></label>}</div><div className="secondary-image-manager"><div><span className="eyebrow">BELOW THE BANNER</span><h3>Secondary image</h3><p>Replace the image and edit its title from this screen.</p></div><label>Image title<input className="secondary-image-title" value={secondaryTitle} onChange={event => setSecondaryTitle(event.target.value)} placeholder="Craft N Sofa collection" /></label>{content.secondary_image_url ? <div className="secondary-image-row"><div><img src={content.secondary_image_url} alt={secondaryTitle || 'Secondary storefront preview'} /><small>{secondaryTitle || 'Craft N Sofa collection'}</small></div><label className="secondary-image-drop compact"><Upload size={18} /><b>Replace image</b><input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => uploadSecondary(event.target.files?.[0])} /></label><button className="danger-button" onClick={onClearSecondary} disabled={busy}>Remove image</button></div> : <label className="secondary-image-drop"><ImagePlus size={22} /><b>Upload image below the banner</b><small>JPG, PNG or WebP up to 10 MB</small><input type="file" accept="image/png,image/jpeg,image/webp" onChange={event => uploadSecondary(event.target.files?.[0])} /></label>}</div></>;
}
...
function BannerUploadModal({ onClose, onSave }: { onClose: () => void; onSave: (desktopFile: File, mobileFile: File | null, altText: string) => Promise<void> }) {
  ...
  const [busy, setBusy] = useState(false);
  const choose = (next: File | undefined, mobile: boolean) => {
    if (!next || !next.type.startsWith('image/') || next.size > 10 * 1024 * 1024) return;
    const preview = URL.createObjectURL(next);
    if (mobile) { setMobileFile(next); setMobilePreview(preview); } else { setDesktopFile(next); setDesktopPreview(preview); }
  };
  const save = async () => { if (!desktopFile) return; setBusy(true); try { await onSave(desktopFile, mobileFile, altText); } finally { setBusy(false); } };
  return <Modal title="Add main homepage banner" onClose={onClose}><p className="modal-intro">Upload separate artwork for the top homepage banner. Desktop uses 1500 × 500 and mobile uses 500 × 500.</p><label>Banner name<input value={altText} onChange={event => setAltText(event.target.value)} placeholder="e.g. Summer collection" /></label><div className="banner-upload-pair"><div><b>Desktop banner · 1500 × 500</b><div className="dropzone banner-dropzone" onClick={() => document.getElementById('desktop-banner-file')?.click()} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); choose(event.dataTransfer.files[0], false); }}>{desktopPreview ? <img src={desktopPreview} alt="Desktop banner preview" /> : <><CloudUpload size={26} /><span>Drop desktop image or click to browse</span></>}<input id="desktop-banner-file" type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={event => choose(event.target.files?.[0], false)} /></div></div><div><b>Mobile banner · 500 × 500</b><div className="dropzone banner-dropzone banner-mobile-dropzone" onClick={() => document.getElementById('mobile-banner-file')?.click()} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); choose(event.dataTransfer.files[0], true); }}>{mobilePreview ? <img src={mobilePreview} alt="Mobile banner preview" /> : <><CloudUpload size={26} /><span>Optional: drop mobile image</span></>}<input id="mobile-banner-file" type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={event => choose(event.target.files?.[0], true)} /></div></div></div><small className="settings-help">The desktop image is required. If no mobile image is uploaded, the desktop image will be used on phones.</small><div className="modal-actions"><button className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={!desktopFile || busy} onClick={save}>{busy ? 'Uploading…' : 'Add banner'}</button></div></Modal>
}
function BannerEditModal({ banner, onClose, onSave }: { banner: Banner; onClose: () => void; onSave: (banner: Banner, desktopFile: File | null, mobileFile: File | null, removeMobile: boolean, altText: string) => Promise<void> }) {
  const [desktopFile, setDesktopFile] = useState<File | null>(null);
  const [mobileFile, setMobileFile] = useState<File | null>(null);
  const [desktopPreview, setDesktopPreview] = useState(banner.image_url);
  const [mobilePreview, setMobilePreview] = useState(banner.mobile_image_url || banner.image_url);
  const [removeMobile, setRemoveMobile] = useState(false);
  const [altText, setAltText] = useState(banner.alt_text);
  const [busy, setBusy] = useState(false);
  const choose = (next: File | undefined, mobile: boolean) => { if (!next || !next.type.startsWith('image/') || next.size > 10 * 1024 * 1024) return; const preview = URL.createObjectURL(next); if (mobile) { setMobileFile(next); setMobilePreview(preview); setRemoveMobile(false); } else { setDesktopFile(next); setDesktopPreview(preview); } };
  const save = async () => { setBusy(true); try { await onSave(banner, desktopFile, mobileFile, removeMobile, altText); } finally { setBusy(false); } };
  return <Modal title="Edit homepage banner" onClose={onClose}><p className="modal-intro">Replace the desktop image, mobile image, or banner title. Existing images stay unchanged unless you select a replacement.</p><label>Banner name<input value={altText} onChange={event => setAltText(event.target.value)} /></label><div className="banner-upload-pair"><div><b>Desktop banner · 1500 × 500</b><div className="dropzone banner-dropzone" onClick={() => document.getElementById('edit-desktop-banner-file')?.click()} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); choose(event.dataTransfer.files[0], false); }}><img src={desktopPreview} alt="Desktop banner" /><input id="edit-desktop-banner-file" type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={event => choose(event.target.files?.[0], false)} /></div></div><div><b>Mobile banner · 500 × 500</b><div className="dropzone banner-dropzone banner-mobile-dropzone" onClick={() => document.getElementById('edit-mobile-banner-file')?.click()} onDragOver={event => event.preventDefault()} onDrop={event => { event.preventDefault(); choose(event.dataTransfer.files[0], true); }}><img src={mobilePreview} alt="Mobile banner" /><input id="edit-mobile-banner-file" type="file" accept="image/png,image/jpeg,image/webp" hidden onChange={event => choose(event.target.files?.[0], true)} /></div><label className="content-block-active"><input type="checkbox" checked={removeMobile} onChange={event => setRemoveMobile(event.target.checked)} /> Use desktop image on mobile</label></div></div><div className="modal-actions"><button className="secondary-button" onClick={onClose}>Cancel</button><button className="primary-button" disabled={busy} onClick={save}>{busy ? 'Saving…' : 'Save banner changes'}</button></div></Modal>;
}
...
function CategoryListView({ categories, onAdd, onDelete, onReplaceImage }: { categories: Category[]; onAdd: () => void; onDelete: (id: string) => void; onReplaceImage: (id: string, file: File) => void | Promise<void> }) {
  return <><section className="welcome-row"><div><span className="eyebrow">CATALOGUE</span><h2>Category List</h2><p>Organise products into clear collections for your team and customers.</p></div><button className="primary-button" onClick={onAdd}><Plus size={16} /> Add category</button></section><section className="panel category-list-page">{categories.map(category => <div className="category-row" key={category.id}>{category.image_url ? <img className="category-row-thumb" src={category.image_url} alt="" /> : <div className="category-icon"><Tag size={16} /></div>}<div><b>{category.name}</b><small>{category.description || 'No description'}</small></div><span className="status-pill">{category.active ? 'Active' : 'Hidden'}</span><div className="category-row-actions"><input id={`category-image-${category.id}`} type="file" accept="image/*" className="sr-only" onChange={async event => { const file = event.target.files?.[0]; event.currentTarget.value = ''; if (file) await onReplaceImage(category.id, file); }} /><label className="category-replace-image" htmlFor={`category-image-${category.id}`} title="Replace category image"><Upload size={14} /><span>Replace Image</span></label><button className="icon-button danger" onClick={() => onDelete(category.id)} title="Delete category" aria-label={`Delete ${category.name}`}><Trash2 size={15} /></button></div></div>)}{!categories.length && <div className="empty-state">No categories yet. Add your first category.</div>}</section></>;
}
